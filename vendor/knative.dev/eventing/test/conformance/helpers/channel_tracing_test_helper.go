...
// SetupFunc sets up the infrastructure for running tracing tests. It returns the expected trace as
// well as a string that is expected to be in the logger Pod's logs.
type SetupFunc func(t *testing.T, channel *metav1.TypeMeta, client *common.Client, loggerPodName string, tc TracingTestCase) (expected tracinghelper.TestSpanTree, logContents string)
