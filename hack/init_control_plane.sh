...
kubectl --namespace "${CONTROL_PLANE_NAMESPACE}" create secret generic "${CONTROLLER_GSA_SECRET_NAME}" \
  --from-file=key.json="${CONTROLLER_GSA_KEY_TEMP}" --dry-run -o yaml | kubectl apply --filename -
