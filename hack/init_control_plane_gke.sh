...
init_controller_gsa "${PROJECT_ID}" "${CONTROLLER_GSA}"
enable_workload_identity "${PROJECT_ID}" "${CONTROLLER_GSA}" "${CLUSTER_NAME}" "${CLUSTER_LOCATION}" "${CLUSTER_LOCATION_TYPE}"
...
gcloud iam service-accounts add-iam-policy-binding \
  ...
  --member "$MEMBER" "${CONTROLLER_GSA}"@"${PROJECT_ID}".iam.gserviceaccount.com

# Add annotation to Kubernetes service account.
kubectl annotate --overwrite serviceaccount "${K8S_CONTROLLER_SERVICE_ACCOUNT}" iam.gke.io/gcp-service-account="${CONTROLLER_GSA}"@"${PROJECT_ID}".iam.gserviceaccount.com \
