...
- Check authentication configuration is correct for the Control Plane.
  ...
  - If you are using Kubernetes Secret for the Control Plane, refer
    ...
    check the Kubernetes Secret `google-cloud-key` in namespace `cloud-run-events`.
...
namespace `cloud-run-events`, you still get permission related error. To such case,
