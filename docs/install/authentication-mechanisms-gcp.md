...
Refer to the [installation guide](./install-knative-gcp.md) for automated scripts.
...
To ensure Workload Identity is correctly configured for a Google Cloud Service Account (GSA), 
...
1. ***Resources are not READY***
   ...
   * If this CloudAuditLogsSource failed due to the JSON private key stored in the Kubernetes Secret having been revoked
   ...
   * If this CloudAuditLogsSource failed due to the GSA having insufficient permissions
   ...
   * If this CloudAuditLogsSource failed due to the `iam-policy-binding` being setup incorrectly on the Google Service Account (for Workload Identity authentication configuration), 
     the `Condition` `Ready` would look like:
...
1. ***Resources are READY, but can't receive Events***
   ...
   * If this `Deployment` doesn't have minimum availability due to the GSA having insufficient permissions
     (for either Kubernetes Secret authentication configuration or Workload Identity authentication configuration), 
   ...
   ***To solve this issue***, you can:
   ...
   - Check authentication configuration is correct for this resource instance. 
     ...
     - If you are using Kubernetes Secrets for this resource instance, 
     refer [here](./authentication-mechanisms-gcp.md/#kubernetes-secrets) to check the 
...
1. ***Resources are not READY, due to WorkloadIdentityReconcileFailed***
   ...
   * If the `Condition` `Ready` has permission related error message like this:
     ...
     the Control Plane's Google Cloud Service Account `cloud-run-events`, refer to
