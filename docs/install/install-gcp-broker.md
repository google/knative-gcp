...
1.  Annotate the `broker` Kubernetes Service Account with
    ...
    ```shell
    kubectl --namespace events-system  annotate serviceaccount broker iam.gke.io/gcp-service-account=cre-dataplane@${PROJECT_ID}.iam.gserviceaccount.com
    ```
...
1. Create a secret on the Kubernetes cluster with the downloaded key.
   ...
   ```shell
   kubectl --namespace events-system create secret generic google-broker-key --from-file=key.json=cre-dataplane.json
   ```
