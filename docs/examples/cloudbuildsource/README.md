...
1. Enable the `Cloud Build API` on your project, this is needed to submit builds via gcloud cli.

   ```shell
   gcloud services enable cloudbuild.googleapis.com
   ```

1. Follow the `Receiving build notifications` section in [Subscribing to build notifications](https:/cloud.google.com/cloud-build/docs/subscribe-build-notifications#receiving_build_notifications) to set up Cloud Build to emit build notifications.
