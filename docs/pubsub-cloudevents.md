# Pub/Sub Transport Binding for CloudEvents - Version 0.3-wip

## Abstract

The Pub/Sub Transport Binding for CloudEvents defines how events are mapped to Pub/Sub
...
elements defined in the CloudEvents specification are to be used in [Pub/Sub][pubsub] requests and response messages.
...
In the _batched_ content mode several events are batched into a single Pub/Sub
message body using an [event format](#14-event-formats) that
supports batching.
...
All attributes are transferred as metadata without further interpretation.
...
of its choosing, but as the value is transposed into a Pub/Sub message as defined in this
...
the `Content-Type` attribute value. If the value is prefixed with the CloudEvents
...
For the _binary_ mode, the attribute `Content-Type` value is not used.

#### 3.1.2. Event Data Encoding

The [`data` attribute](#22-data-attribute) byte-sequence is used as the Pub/Sub
...
The value for each Pub/Sub attribute is constructed from the respective attribute's
...
Some CloudEvents metadata attributes can contain arbitrary UTF-8 string content,
 Pub/Sub attributes MUST only use
...
be percent-encoded to be represented as Pub/Sub attribute characters. The rules for
...
The [Pub/Sub Attribute `Content-Type`][content-type] header MUST be set to the media type of
...
In the _batched_ content mode several events are batched into a single Pub/Sub Message.
The chosen [event format](#14-event-formats) MUST
...
This example shows two batched CloudEvents published:
