...
func main() {
	...
	targetsCh, err := volume.NewTargetsFromFile(volume.WithPath(env.TargetsConfigPath))
	if err != nil {
		logger.Fatalw("Failed to load targets config", zap.String("path", env.TargetsConfigPath), zap.Error(err))
	}
	targetsWatcher := config.NewTargetsWatcher(targetsCh)

	logger.Info("Starting the broker fanout")

	if _, err := fanout.StartSyncPool(ctx, targetsWatcher, buildPoolOptions(env)...); err != nil {
