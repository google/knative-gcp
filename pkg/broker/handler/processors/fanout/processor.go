...
func (p *Processor) Process(ctx context.Context, event *event.Event) error {
	...
	if !ok {
		...
		logging.FromContext(ctx).Warn("broker no longer exist in the config", zap.String("broker", bk.PersistenceString()))
		return nil
	}
...
func (p *Processor) mergeResults(ctx context.Context, resChs []<-chan *fanoutResult) error {
	...
	count := func(c <-chan *fanoutResult) {
		for fr := range c {
			if fr.err != nil {
				logging.FromContext(ctx).Error("error processing event for fanout target", zap.String("target", fr.targetKey.LogString()))
	...
	logging.FromContext(ctx).Debug("event fanout successful", zap.String("broker", bk.PersistenceString()), zap.Int32("count", passes))
	return nil
}
