...
func NewMultiTopicDecoupleSink(
	...
	return &multiTopicDecoupleSink{
		...
		topics: make(map[config.GCPCellAddressableKey]*pubsub.Topic),
...
type multiTopicDecoupleSink struct {
	...
	topics    map[config.GCPCellAddressableKey]*pubsub.Topic
...
func (m *multiTopicDecoupleSink) Send(ctx context.Context, broker config.GCPCellAddressableKey, event cev2.Event) protocol.Result {
...
func (m *multiTopicDecoupleSink) getTopicForBroker(ctx context.Context, broker config.GCPCellAddressableKey) (*pubsub.Topic, error) {
...
func (m *multiTopicDecoupleSink) updateTopicForBroker(ctx context.Context, broker config.GCPCellAddressableKey) (*pubsub.Topic, error) {
	m.topicsMut.Lock()
...
func (m *multiTopicDecoupleSink) getTopicIDForBroker(ctx context.Context, broker config.GCPCellAddressableKey) (string, error) {
	brokerConfig, ok := m.brokerConfig.GetGCPAddressableByKey(broker)
	if !ok {
		...
		return "", fmt.Errorf("%q: %w", broker.PersistenceString(), ErrNotFound)
	}
	if brokerConfig.DecoupleQueue == nil || brokerConfig.DecoupleQueue.Topic == "" {
		...
		return "", fmt.Errorf("decouple queue of %q: %w", broker.PersistenceString(), ErrIncomplete)
	}
	if brokerConfig.DecoupleQueue.State != config.State_READY {
		logging.FromContext(ctx).Debug("decouple queue is not ready")
		return "", fmt.Errorf("%q: %w", broker.PersistenceString(), ErrNotReady)
	}
	return brokerConfig.DecoupleQueue.Topic, nil
}

func (m *multiTopicDecoupleSink) getExistingTopic(broker config.GCPCellAddressableKey) (*pubsub.Topic, bool) {
	m.topicsMut.RLock()
