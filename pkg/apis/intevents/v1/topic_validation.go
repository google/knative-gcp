...
func (t *Topic) Validate(ctx context.Context) *apis.FieldError {
	if v, present := t.Annotations[LoggingE2ETestAnnotation]; present {
		...
		logging.FromContext(ctx).Error("Adding log line for the TestCloudLogging E2E test", zap.String(LoggingE2EFieldName, v))
	}
