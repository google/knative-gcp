...
func FromContext(ctx context.Context) *Config {
	x, ok := ctx.Value(brDeliveryCfgKey{}).(*Config)
	if ok {
		return x
	}
	return nil
...
func ToContext(ctx context.Context, c *Config) context.Context {
	return context.WithValue(ctx, brDeliveryCfgKey{}, c)
}
...
func (s *Store) Load() *Config {
	return &Config{
		BrokerDeliverySpecDefaults: s.UntypedLoad(ConfigMapName()).(*Defaults).DeepCopy(),
	}
}
