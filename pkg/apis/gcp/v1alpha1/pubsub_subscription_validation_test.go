...
var (
	fullSpec = PubSubSubscriptionSpec{
...
func TestPubSubCheckImmutableFields(t *testing.T) {
	testCases := map[string]struct {
		orig    *PubSubSubscriptionSpec
		updated PubSubSubscriptionSpec
		allowed bool
	}{
		"nil orig": {
			updated: fullSpec,
	...
	for n, tc := range testCases {
		t.Run(n, func(t *testing.T) {
			var orig *PubSubSubscription
			if tc.orig != nil {
				orig = &PubSubSubscription{
					Spec: *tc.orig,
				}
			}
			updated := &PubSubSubscription{
				Spec: tc.updated,
			}
