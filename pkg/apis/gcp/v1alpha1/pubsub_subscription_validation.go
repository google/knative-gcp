...
func (current *PubSubSubscription) CheckImmutableFields(ctx context.Context, og apis.Immutable) *apis.FieldError {
	original, ok := og.(*PubSubSubscription)
	if !ok {
		return &apis.FieldError{Message: "The provided original was not a PubSubSubscription"}
	}
	if original == nil {
		return nil
	}
