...
func TestAllCases(t *testing.T) {
	...
	table := TableTest{{
		...
		WantEvents: []string{
			...
			Eventf(corev1.EventTypeWarning, "UpdateFailed", `Failed to update status for "%s": invalid value: test@test, serviceAccount should have format: [A-Za-z0-9-]+@[A-Za-z0-9-]+\.iam.gserviceaccount.com: spec.serviceAccount`, schedulerName),
		},
		WantErr: true,
	}, {
