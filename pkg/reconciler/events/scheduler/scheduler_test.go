	...
	table := TableTest{{
		...
		WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
			Object: NewScheduler(schedulerName, testNS,
				...
				WithSchedulerSchedule(onceAMinuteSchedule),
				WithInitSchedulerConditions,
				WithSchedulerTopicFalse("TopicNotReady", `Topic "my-test-scheduler" did not expose projectid`),
				WithSchedulerFinalizers(finalizerName),
			),
		}},
		WantEvents: []string{
		...
		WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
			Object: NewScheduler(schedulerName, testNS,
				...
				WithSchedulerSchedule(onceAMinuteSchedule),
				WithInitSchedulerConditions,
				WithSchedulerTopicFalse("TopicNotReady", `Topic "my-test-scheduler" did not expose topicid`),
				WithSchedulerFinalizers(finalizerName),
			),
		}},
		WantEvents: []string{
		...
		WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
			Object: NewScheduler(schedulerName, testNS,
				...
				WithSchedulerSchedule(onceAMinuteSchedule),
				WithInitSchedulerConditions,
				WithSchedulerTopicFalse("TopicNotReady", `Topic "my-test-scheduler" mismatch: expected "scheduler-test-scheduler-uid" got "garbaaaaage"`),
				WithSchedulerFinalizers(finalizerName),
			),
		}},
		WantEvents: []string{
		...
		WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
			Object: NewScheduler(schedulerName, testNS,
				...
				WithSchedulerFinalizers(finalizerName),
				WithInitSchedulerConditions,
				WithSchedulerTopicFalse("PublisherStatus", "Publisher has no Ready type status"),
			),
		}},
		WantEvents: []string{
	...
	},
		{
			...
			Objects: []runtime.Object{
				...
				NewPullSubscriptionWithNoDefaults(schedulerName, testNS, WithPullSubscriptionFalse()),
				newSink(),
			},
			Key:     testNS + "/" + schedulerName,
			WantErr: true,
			WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
				Object: NewScheduler(schedulerName, testNS,
					...
					WithInitSchedulerConditions,
					WithSchedulerTopicReady(testTopicID, testProject),
					WithSchedulerPullSubscriptionFalse("InvalidSink", `failed to get ref &ObjectReference{Kind:Sink,Namespace:testnamespace,Name:sink,UID:,APIVersion:testing.cloud.google.com/v1alpha1,ResourceVersion:,FieldPath:,}: sinks.testing.cloud.google.com "sink" not found`),
				),
			}},
			WantEvents: []string{
			...
			WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
				Object: NewScheduler(schedulerName, testNS,
					...
					WithSchedulerTopicFalse("TopicDeleted", fmt.Sprintf("Successfully deleted Topic: %s", schedulerName)),
					WithSchedulerPullSubscriptionFalse("PullSubscriptionDeleted", fmt.Sprintf("Successfully deleted PullSubscription: %s", schedulerName)),
					WithSchedulerDeletionTimestamp,
				),
			}},
