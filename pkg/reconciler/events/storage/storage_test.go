	...
	table := TableTest{{
		...
		WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
			Object: NewStorage(storageName, testNS,
				...
				WithStorageSink(sinkGVK, sinkName),
				WithInitStorageConditions,
				WithStorageTopicFailed("TopicNotReady", `Topic "my-test-storage" did not expose projectid`),
				WithStorageFinalizers(finalizerName),
			),
		}},
		WantEvents: []string{
		...
		WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
			Object: NewStorage(storageName, testNS,
				...
				WithStorageSink(sinkGVK, sinkName),
				WithInitStorageConditions,
				WithStorageTopicFailed("TopicNotReady", `Topic "my-test-storage" did not expose topicid`),
				WithStorageFinalizers(finalizerName),
			),
		}},
		WantEvents: []string{
		...
		WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
			Object: NewStorage(storageName, testNS,
				...
				WithStorageSink(sinkGVK, sinkName),
				WithInitStorageConditions,
				WithStorageTopicFailed("TopicNotReady", `Topic "my-test-storage" mismatch: expected "storage-test-storage-uid" got "garbaaaaage"`),
				WithStorageFinalizers(finalizerName),
			),
		}},
		WantEvents: []string{
		...
		WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
			Object: NewStorage(storageName, testNS,
				...
				WithStorageSink(sinkGVK, sinkName),
				WithInitStorageConditions,
				WithStorageTopicFailed("PublisherStatus", "Publisher has no Ready type status"),
				WithStorageFinalizers(finalizerName),
			),
		}},
		WantEvents: []string{
	...
	},
		{
			...
			WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
				Object: NewStorage(storageName, testNS,
					...
					WithStorageTopicReady(testTopicID),
					WithStorageProjectID(testProject),
					WithStoragePullSubscriptionFailed("InvalidSink", `failed to get ref &ObjectReference{Kind:Sink,Namespace:testnamespace,Name:sink,UID:,APIVersion:testing.cloud.google.com/v1alpha1,ResourceVersion:,FieldPath:,}: sinks.testing.cloud.google.com "sink" not found`),
				),
			}},
			WantEvents: []string{
		...
		},
		{
			...
			WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
				Object: NewStorage(storageName, testNS,
					...
					WithStorageTopicFailed("TopicDeleted", fmt.Sprintf("Successfully deleted Topic: %s", storageName)),
					WithStoragePullSubscriptionFailed("PullSubscriptionDeleted", fmt.Sprintf("Successfully deleted PullSubscription: %s", storageName)),
					WithDeletionTimestamp()),
			}},
		}}
