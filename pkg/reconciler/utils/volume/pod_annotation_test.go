...
func TestUpdateAnnotation(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err := UpdateVolumeGeneration(context.TODO(), client, listers.GetPodLister(), testNS, ls)
...
func getPods(t *testing.T, client kubernetes.Interface, namespaces ...string) []corev1.Pod {
	...
	for _, namespace := range namespaces {
		gotPods, err := client.CoreV1().Pods(namespace).List(context.TODO(), metav1.ListOptions{})
		if err != nil {
