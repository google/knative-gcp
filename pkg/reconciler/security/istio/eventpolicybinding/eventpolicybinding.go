...
import (
	...
	"github.com/google/knative-gcp/pkg/reconciler/security/istio/eventpolicybinding/resources"
)
...
type Reconciler struct {
	*reconciler.Base
	...
	eventPolicyLister       securitylisters.EventPolicyLister
	httpPolicyLister        securitylisters.HTTPPolicyLister
	httpPolicyBindingLister securitylisters.HTTPPolicyBindingLister
...
func (r *Reconciler) reconcileHTTPPolicyBinding(
	...
	if apierrs.IsNotFound(err) {
		existing, err = r.RunClientSet.SecurityV1alpha1().HTTPPolicyBindings(desired.Namespace).Create(desired)
		if err != nil {
	...
	if !equality.Semantic.DeepEqual(desired.Spec, existing.Spec) {
		...
		existing, err = r.RunClientSet.SecurityV1alpha1().HTTPPolicyBindings(cp.Namespace).Update(cp)
		if err != nil {
...
func (r *Reconciler) reconcileHTTPPolicy(
	...
	if apierrs.IsNotFound(err) {
		existing, err = r.RunClientSet.SecurityV1alpha1().HTTPPolicies(desired.Namespace).Create(desired)
		if err != nil {
	...
	if !equality.Semantic.DeepEqual(desired.Spec, existing.Spec) {
		...
		cp := existing.DeepCopy()
		cp.Spec = desired.Spec
		existing, err = r.RunClientSet.SecurityV1alpha1().HTTPPolicies(cp.Namespace).Update(cp)
		if err != nil {
