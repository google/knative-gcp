	...
	table := TableTest{{
		...
		WantStatusUpdates: []clientgotesting.UpdateActionImpl{{
			Object: NewPolicyBinding(testBindingName, testNamespace,
				...
				WithPolicyBindingStatusFailure("GetPolicyFailure", `eventpolicy.security.knative.dev "testpolicy" not found`),
			).AsEventPolicyBinding(),
		}},
		WantEvents: []string{
			Eventf(corev1.EventTypeWarning, "InternalError", `failed to get EventPolicy: eventpolicy.security.knative.dev "testpolicy" not found`),
		},
		WantErr: true,
	}, {
