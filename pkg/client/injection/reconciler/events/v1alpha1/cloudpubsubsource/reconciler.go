...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.CloudPubSubSources(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.CloudPubSubSource, desired *v1alpha1.CloudPubSubSource) error {
	...
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			existing, err = r.Client.EventsV1alpha1().CloudPubSubSources(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.EventsV1alpha1().CloudPubSubSources(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.CloudPubSubSource) (*v1alpha1.CloudPubSubSource, error) {
	...
	actual, err := r.Lister.CloudPubSubSources(resource.Namespace).Get(resource.Name)
	if err != nil {
		return resource, err
	}
	...
	resource, err = r.Client.EventsV1alpha1().CloudPubSubSources(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
