...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.CloudSchedulerSources(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.CloudSchedulerSource, desired *v1alpha1.CloudSchedulerSource) error {
	...
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			existing, err = r.Client.EventsV1alpha1().CloudSchedulerSources(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.EventsV1alpha1().CloudSchedulerSources(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.CloudSchedulerSource) (*v1alpha1.CloudSchedulerSource, error) {
	...
	actual, err := r.Lister.CloudSchedulerSources(resource.Namespace).Get(resource.Name)
	if err != nil {
		return resource, err
	}
	...
	resource, err = r.Client.EventsV1alpha1().CloudSchedulerSources(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
