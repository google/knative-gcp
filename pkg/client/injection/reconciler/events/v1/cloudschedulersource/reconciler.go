...
import (
	...
	eventsv1 "github.com/google/knative-gcp/pkg/client/listers/events/v1"
	zap "go.uber.org/zap"
...
// controller reconciling v1.CloudSchedulerSource.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.CloudSchedulerSource. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.CloudSchedulerSource) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.CloudSchedulerSource.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.CloudSchedulerSource. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.CloudSchedulerSource) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.CloudSchedulerSource if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.CloudSchedulerSource.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.CloudSchedulerSource) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.CloudSchedulerSource if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1.CloudSchedulerSource.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1.CloudSchedulerSource) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.CloudSchedulerSource) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.CloudSchedulerSource resources.
type reconcilerImpl struct {
	...
	Lister eventsv1.CloudSchedulerSourceLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister eventsv1.CloudSchedulerSourceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1.CloudSchedulerSource, desired *v1.CloudSchedulerSource) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.EventsV1().CloudSchedulerSources(desired.Namespace)
		...
		updater := r.Client.EventsV1().CloudSchedulerSources(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.CloudSchedulerSource) (*v1.CloudSchedulerSource, error) {
	...
	patcher := r.Client.EventsV1().CloudSchedulerSources(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.CloudSchedulerSource) (*v1.CloudSchedulerSource, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.CloudSchedulerSource, reconcileEvent reconciler.Event) (*v1.CloudSchedulerSource, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
