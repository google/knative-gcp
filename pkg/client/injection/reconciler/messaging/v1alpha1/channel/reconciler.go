...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.Channels(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.Channel, desired *v1alpha1.Channel) error {
	...
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			existing, err = r.Client.MessagingV1alpha1().Channels(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.MessagingV1alpha1().Channels(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.Channel) (*v1alpha1.Channel, error) {
	...
	actual, err := r.Lister.Channels(resource.Namespace).Get(resource.Name)
	if err != nil {
		return resource, err
	}
	...
	resource, err = r.Client.MessagingV1alpha1().Channels(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
