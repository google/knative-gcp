...
type CloudStorageSourceInformer interface {
	...
	Lister() v1.CloudStorageSourceLister
}

type cloudStorageSourceInformer struct {
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return client.EventsV1().CloudStorageSources(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.EventsV1().CloudStorageSources(namespace).Watch(context.TODO(), options)
			},
		},
		&eventsv1.CloudStorageSource{},
		resyncPeriod,
		indexers,
	)
}

func (f *cloudStorageSourceInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *cloudStorageSourceInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&eventsv1.CloudStorageSource{}, f.defaultInformer)
}

func (f *cloudStorageSourceInformer) Lister() v1.CloudStorageSourceLister {
	return v1.NewCloudStorageSourceLister(f.Informer().GetIndexer())
}
