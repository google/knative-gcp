...
// List lists all CloudPubSubSources in the indexer.
func (s *cloudPubSubSourceLister) List(selector labels.Selector) (ret []*v1beta1.CloudPubSubSource, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.CloudPubSubSource))
	})
	return ret, err
}
...
// List lists all CloudPubSubSources in the indexer for a given namespace.
func (s cloudPubSubSourceNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.CloudPubSubSource, err error) {
...
// Get retrieves the CloudPubSubSource from the indexer for a given namespace and name.
func (s cloudPubSubSourceNamespaceLister) Get(name string) (*v1beta1.CloudPubSubSource, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("cloudpubsubsource"), name)
	}
	return obj.(*v1beta1.CloudPubSubSource), nil
}
