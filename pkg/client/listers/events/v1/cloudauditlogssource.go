...
// List lists all CloudAuditLogsSources in the indexer for a given namespace.
func (s cloudAuditLogsSourceNamespaceLister) List(selector labels.Selector) (ret []*v1.CloudAuditLogsSource, err error) {
...
// Get retrieves the CloudAuditLogsSource from the indexer for a given namespace and name.
func (s cloudAuditLogsSourceNamespaceLister) Get(name string) (*v1.CloudAuditLogsSource, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("cloudauditlogssource"), name)
	}
	return obj.(*v1.CloudAuditLogsSource), nil
}
