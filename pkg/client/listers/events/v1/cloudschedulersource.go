...
// List lists all CloudSchedulerSources in the indexer.
func (s *cloudSchedulerSourceLister) List(selector labels.Selector) (ret []*v1.CloudSchedulerSource, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.CloudSchedulerSource))
	})
	return ret, err
}
...
// List lists all CloudSchedulerSources in the indexer for a given namespace.
func (s cloudSchedulerSourceNamespaceLister) List(selector labels.Selector) (ret []*v1.CloudSchedulerSource, err error) {
...
// Get retrieves the CloudSchedulerSource from the indexer for a given namespace and name.
func (s cloudSchedulerSourceNamespaceLister) Get(name string) (*v1.CloudSchedulerSource, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("cloudschedulersource"), name)
	}
	return obj.(*v1.CloudSchedulerSource), nil
}
