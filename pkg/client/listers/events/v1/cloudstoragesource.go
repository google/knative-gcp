...
// List lists all CloudStorageSources in the indexer.
func (s *cloudStorageSourceLister) List(selector labels.Selector) (ret []*v1.CloudStorageSource, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.CloudStorageSource))
	})
	return ret, err
}
...
// List lists all CloudStorageSources in the indexer for a given namespace.
func (s cloudStorageSourceNamespaceLister) List(selector labels.Selector) (ret []*v1.CloudStorageSource, err error) {
...
// Get retrieves the CloudStorageSource from the indexer for a given namespace and name.
func (s cloudStorageSourceNamespaceLister) Get(name string) (*v1.CloudStorageSource, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("cloudstoragesource"), name)
	}
	return obj.(*v1.CloudStorageSource), nil
}
