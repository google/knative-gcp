...
// NotificationJobLabels creates a label to find a job again.
...
func NotificationJobLabels(owner kmeta.OwnerRefable, action string) map[string]string {
	return operations.JobLabels("notification", owner.GetObjectMeta().GetName(), owner.GetGroupVersionKind().Kind, action)
}
